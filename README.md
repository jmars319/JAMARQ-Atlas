...
Atlas by Tenra is a local-first operator dashboard for mapping open work across client systems, software suites, experiments, business infrastructure, and outlier repositories.
...
- Repos view for importing registry-derived Repo Operations Snapshot v1 packets, reconciling repo universe, GitHub bindings, local Git status, safe fetch/pull/verification workflow runs, and Planning follow-up.
...
- Settings & Connections Center for local workspace labels, Calibration Operations, and integration-readiness status without storing secrets.
...
- Local Repo Operations storage for imported advisory repo snapshots, filters, Planning evidence links, and redacted repo workflow run history.
...
5. Before restore testing or large data cleanup, use Data Center JSON export or Settings Sync snapshots. Calibration progress and non-secret credential references are included in both.
