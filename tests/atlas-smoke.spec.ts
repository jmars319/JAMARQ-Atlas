...
test('operator can edit manual state and manage writing drafts', async ({ page }) => {
  ...
  const reviewNote = reviewItem.locator('textarea[aria-label^="Review note for"]')
  await expect(reviewNote).toHaveCount(1)
  await reviewItem.getByRole('button', { name: 'Start session' }).click()
  await expect(page.getByText('Review session started locally.')).toBeVisible()
  await reviewNote.fill('E2E review follow-up note')
  await reviewItem.getByRole('button', { name: 'Add review note' }).click()
  await expect(page.getByText('Review note captured locally.')).toBeVisible()
  await reviewNote.fill('E2E planning note from review')
  await reviewItem.getByRole('button', { name: 'Create planning note' }).click()
  ...
  await expectSettingsConnectionStatus(page, 'Vercel Deployment Evidence', /Missing|Available/)
